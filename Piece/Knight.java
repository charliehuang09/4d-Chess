...
public class Knight extends Piece {
    ...
    public ArrayList<Position> getValidMoves(BoardSquare[][] board) {
        ...
        for (int i = 0; i < 8; i++) {
            if (inBound(this.position.getX() + dx[i], this.position.getY() + dy[i], board)
                    && !board[this.position.getX() + dx[i]][this.position.getY() + dy[i]].isNull()
                    && board[this.position.getX() + dx[i]][this.position.getY() + dy[i]].getPlayer() != player)
                output.add(new Position(this.position.getX() + dx[i], this.position.getY() + dy[i]));
        }
        return output;
    }
    ...
    public boolean isValidMove(Position position, BoardSquare[][] board) {
        ...
        int[] dy = new int[] { -1, 1, -1, 1, 2, 2, -2, -2 };
        for (int i = 0; i < 8; i++) {
            if (this.position.getX() + dx[i] == position.getX() && this.position.getY() + dy[i] == position.getY())
                return true;
        }
        return false;
    }
