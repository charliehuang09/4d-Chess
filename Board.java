...
public class Board {
    ...
    public void nullify() { // males all the needed elements need
        // EXPLANATION
        // a method that goes from the board from length to right and makes the need
        // elements in columns = null
        for (int r = 0; r < board.length; r++) {
            if (r < 3 || r >= 11) {
                for (int c = 0; c < 3; c++) {
                    board[r][c].startUpdate(new NullSquare(), true); // replace null with proposed nothing class if
                                                                     // needd
                }
